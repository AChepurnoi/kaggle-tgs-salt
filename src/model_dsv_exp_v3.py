...
class FusionLayer(nn.Module):
    ...
    def _reset_parameters(self):
        torch.nn.init.constant_(self.weights, 1 / self.nums)
...
class AttentionUnet(nn.Module):
    ...
    def __init__(self, n_classes=1, is_deconv=True, in_channels=3, is_batchnorm=True):
        ...
        self.c4 = nn.Conv2d(2, 1, kernel_size=1, padding=0)
        self.c3 = nn.Conv2d(3, 1, kernel_size=1, padding=0)
        self.c2 = nn.Conv2d(4, 1, kernel_size=1, padding=0)
        self.c1 = nn.Conv2d(5, 1, kernel_size=1, padding=0)

        self.fusion = FusionLayer(nums=5)
    ...
    def forward(self, inputs):
        ...
        conv2 = F.max_pool2d(self.conv2(conv1), kernel_size=2)  # 256 x 64 x 64
        ...
        dsv5_f = dsv5
        dsv4 = self.dsv4(up4)
        dsv4_f = self.c4(torch.cat([dsv5_f, dsv4], dim=1))
        dsv3 = self.dsv3(up3)
        dsv3_f = self.c3(torch.cat([dsv5_f, dsv4_f, dsv3], dim=1))
        dsv2 = self.dsv2(up2)
        dsv2_f = self.c2(torch.cat([dsv5_f, dsv4_f, dsv3_f, dsv2], dim=1))
        dsv1 = self.dsv1(up1)
        dsv1_f = self.c1(torch.cat([dsv5_f, dsv4_f, dsv3_f, dsv2_f, dsv1], dim=1))

        mask_bool = self.mask_clf(F.dropout2d(center, p=DROPOUT_RATE, training=self.training))

        final = self.fusion([
            F.dropout2d(dsv5_f, p=DROPOUT_RATE, training=self.training),
            F.dropout2d(dsv4_f, p=DROPOUT_RATE, training=self.training),
            F.dropout2d(dsv3_f, p=DROPOUT_RATE, training=self.training),
            F.dropout2d(dsv2_f, p=DROPOUT_RATE, training=self.training),
            F.dropout2d(dsv1_f, p=DROPOUT_RATE, training=self.training)
        ])

        return final, [dsv1_f, dsv2_f, dsv3_f, dsv4_f, dsv5_f], mask_bool
